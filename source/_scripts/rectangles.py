...
left = p.quad(left='xleft', right='xright', top='yleft',
              bottom='bottom', color='red', alpha=.5, source=source)
right = p.quad(left='xleft', right='xright', top='yright',
               bottom='bottom', color='green', alpha=.5, source=source)
right.visible = False

radio = RadioButtonGroup(labels=['Gauche', 'Droite'], active=0)
slider = Slider(start=10, end=100, value=10, step=1,
                title='Nombre de rectangles')

radio_callback = CustomJS(args=dict(radio=radio, left=left, right=right), code="""
    switch(radio.active){
        case 0:
            left.visible = true;
            right.visible = false;
...
show(column(p, column(radio, slider)))
